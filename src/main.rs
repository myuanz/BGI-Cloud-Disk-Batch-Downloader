...
async fn find_all_files(
    sess: &reqwest::Client, base_rows: &Vec<Value>, share_event_id: &str, tab: usize
...
fn check_os_and_aria2c() -> String {
    ...
    match version {
        Ok(v) => {
            if !std::str::from_utf8(&v.stdout).unwrap().contains("aria2 version") {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let all_files: Vec<DownloadArgs> = find_all_files(&sess, base_rows, share_event_id,0).await.unwrap();
